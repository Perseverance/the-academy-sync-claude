    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="max-w-3xl w-full text-center">
        <header className="mb-12 flex flex-col items-center">
          ...
          <button
            onClick={signIn}
            className="btn-academy-primary text-lg px-8 py-3" // Using custom class
            disabled={isLoading}
          >
            {isLoading ? (
          ...
          {[
            ...
            <Card key={idx} className="bg-card border-border shadow-sm hover:shadow-lg transition-shadow duration-200">
              <CardHeader className="items-center">
                <item.icon className="w-10 h-10 text-accent mb-3" /> {/* Vibrant Teal for icons */}
                <CardTitle className="text-xl font-brand text-primary">{item.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <CardDescription className="text-base text-foreground/70 font-body">{item.desc}</CardDescription>
              </CardContent>
            </Card>
          ))}
        </div>
