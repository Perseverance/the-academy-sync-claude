...
# Check if we're using environment file or need to fetch from Secret Manager
ENV_FILE_PATH="$WEB_DIR/.env.$ENVIRONMENT"
if [ -f "$ENV_FILE_PATH" ]; then
    print_info "Environment file found. OAuth client IDs will be loaded from: .env.$ENVIRONMENT"
    # We'll use the values from the env file during build
    GOOGLE_CLIENT_ID=""
    STRAVA_CLIENT_ID=""
else
    # Get OAuth client IDs from Google Secret Manager
    print_info "No environment file found. Fetching OAuth client IDs from Secret Manager..."
...
if [ ! -f "$ENV_FILE_PATH" ]; then
    print_warning "Environment file not found: $ENV_FILE_PATH"
    print_info "Creating temporary .env.production.local with values from Terraform/Secret Manager..."
    
    # Fall back to creating temporary file with fetched values
    cat > .env.production.local <<EOF
# Temporary build-time configuration for ${ENVIRONMENT} environment
# This file is automatically generated and cleaned up by deploy-frontend.sh
...
EOF
    ...
    print_info "Using environment file: $ENV_FILE_PATH"
    cp "$ENV_FILE_PATH" .env.production.local
    
    # Optionally override specific values if they were fetched from Terraform/Secret Manager
    # This allows the deployment script to update values while keeping others from the env file
    if [ -n "$BACKEND_API_URL" ]; then
        print_info "Overriding NEXT_PUBLIC_API_URL with: $BACKEND_API_URL"
        # Use sed to update the value in .env.production.local
        if grep -q "^NEXT_PUBLIC_API_URL=" .env.production.local; then
            sed -i.bak "s|^NEXT_PUBLIC_API_URL=.*|NEXT_PUBLIC_API_URL=$BACKEND_API_URL|" .env.production.local
        else
            echo "NEXT_PUBLIC_API_URL=$BACKEND_API_URL" >> .env.production.local
        fi
        rm -f .env.production.local.bak
    fi
fi

# Display the environment variables being used
print_info "Environment variables for build:"
grep "^NEXT_PUBLIC_" .env.production.local | while read -r line; do
    echo "  $line"
done
...
if [ -f "build-static.sh" ]; then
    ...
    if ./build-static.sh; then
        ...
        rm -f .env.production.local  # Clean up temp file
        exit 1
    fi
else
...
print_info "Cleaned up temporary .env.production.local file"
